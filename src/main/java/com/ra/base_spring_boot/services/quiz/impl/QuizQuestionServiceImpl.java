package com.ra.base_spring_boot.services.quiz.impl;
...
import com.ra.base_spring_boot.repository.quiz.IQuizQuestionRepository;
import com.ra.base_spring_boot.services.quiz.IQuizQuestionService;
...
public class QuizQuestionServiceImpl implements IQuizQuestionService {
    ...
    public List<QuizQuestionDTO> getByQuiz(Long quizId) {
        List<QuizQuestion> items = quizQuestionRepository.findByQuiz_IdOrderByOrderIndexAscIdAsc(
                java.util.Objects.requireNonNull(quizId, "quizId must not be null"));
        return items.stream().map(this::toDTO).collect(Collectors.toList());
    }

    @Override
    public QuizQuestionDTO create(CreateQuizQuestionRequest request) {
        LessonQuiz quiz = lessonQuizRepository
        ...
        Question question = questionRepository
                .findById(java.util.Objects.requireNonNull(request.getQuestionId(), "questionId must not be null"))
    ...
    public void delete(Long id) {
        QuizQuestion entity = quizQuestionRepository
                .findById(java.util.Objects.requireNonNull(id, "id must not be null"))
