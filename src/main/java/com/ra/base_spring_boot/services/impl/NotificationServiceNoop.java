...
public class NotificationServiceNoop implements NotificationService {
    ...
    public void sendAccountCreatedEmail(String to, String username) {
        ...
        try {
            EmailDTO emailDTO = new EmailDTO(to, "Chào mừng bạn đến với hệ thống", "user_created", Map.of("username", username));
    ...
    public void sendTeacherAccountCreatedEmail(String to, String username, String tempPassword) {
        if (!sendOnAdminCreate) {
            log.debug("Skipping teacher account email for {} (sendOnAdminCreate=false)", to);
            return;
        }
        try {
            EmailDTO emailDTO = new EmailDTO(
                    ...
                    Map.of("username", username, "tempPassword", tempPassword, "email", to)
            );
            gmailService.sendEmail(emailDTO);
            log.info("Admin-created teacher account email sent to {}", to);
        } catch (Exception e) {
            log.error("Failed to send teacher account email to {}: {}", to, e.getMessage(), e);
        }
    }
    ...
    public void sendAddedToClassEmail(String to, String className) {
        ...
        try {
            EmailDTO emailDTO = new EmailDTO(to, "Bạn đã được thêm vào lớp học", "added_to_class", Map.of("className", className));
            gmailService.sendEmail(emailDTO);
    ...
    public void sendNewCourseEmail(String to, String courseName, String className) {
        ...
        try {
            EmailDTO emailDTO = new EmailDTO(to, "Khóa học mới", "new_course", Map.of("courseName", courseName, "className", className));
            gmailService.sendEmail(emailDTO);
    ...
    public void sendNewExamEmail(String to, String examName, String className) {
        ...
        try {
            EmailDTO emailDTO = new EmailDTO(to, "Bài kiểm tra mới", "new_exam", Map.of("examName", examName, "className", className));
            gmailService.sendEmail(emailDTO);
