...
public class ScheduleItemController {
        ...
        @Operation(summary = "Tạo thời khóa biểu tự động", description = "ADMIN tạo thời khóa biểu theo course + class")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Tạo thành công", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = ScheduleItemResponseDTO.class)))),
        ...
        @PostMapping("/manual")
        @PreAuthorize("hasAuthority('ROLE_ADMIN')")
        @Operation(summary = "Tạo thời khóa biểu thủ công", description = "ADMIN chọn ngày trong tuần + period")
        ...
        @PreAuthorize("hasAuthority('ROLE_ADMIN')")
        @Operation(summary = "Xóa thời khóa biểu theo class_course", description = "Xóa toàn bộ lịch của 1 khóa học trong 1 lớp")
        ...
        @PreAuthorize("hasAuthority('ROLE_ADMIN')")
        @Operation(summary = "Cập nhật buổi học", description = "Đổi ngày, period, trạng thái")
        ...
        // ===================================================================
        // 7) CẬP NHẬT LỊCH THEO TUẦN
        // ===================================================================
        @PutMapping("/week")
        @PreAuthorize("hasAuthority('ROLE_ADMIN')")
        @Operation(summary = "Cập nhật lịch theo tuần", description = "Cập nhật lịch cho một tuần cụ thể mà không ảnh hưởng các tuần khác")
        public ResponseEntity<List<ScheduleItemResponseDTO>> updateWeekSchedule(
                        @RequestBody @Valid UpdateWeekScheduleRequestDTO req) {
                return ResponseEntity.ok(
                                scheduleItemService.updateWeekSchedule(req));
        }
}
